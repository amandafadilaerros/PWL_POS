    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ url('/stok/'.$stok->stok_id) }}" class="form-horizontal">
            ...
            <div class="form-group row">
                ...
                <div class="col-11">
                    <select class="form-control" id="barang_id" name="barang_id" required>
                        ...
                        <option value="{{ $item->user_id }}" @if($item->user_id == $stok->user_id) selected @endif>{{ $item->username }}</option>
                        @endforeach
                    </select>
                    @error('barang_id')
                    <small class="form-text text-danger">{{ $message }}</small>
            ...
            <div class="form-group row">
                ...
                <div class="col-11">
                    <?php
                    // Memformat tanggal dari variabel PHP ke format dd-mm-YYYY
                    $formattedDate = date('d-m-Y', strtotime($stok->stok_tanggal));
                    ?>
                    <input type="text" class="form-control" id="stok_tanggal" name="stok_tanggal" value="{{ old('stok_tanggal', $formattedDate) }}" required>
                    @error('stok_tanggal')
                    <small class="form-text text-danger">{{ $message }}</small>
                    @enderror
                </div>
